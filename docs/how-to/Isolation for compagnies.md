...
```Bash
sed 's#processing#playground-ns#g' -i ~/rs-workflow-env/apps/prefect3-server-playground/kustomization.yaml
```
...
Edit the values by changing `prefect3server.ops` to `prefect3server.playground` in the file `~/rs-workflow-env/apps/prefect3-worker-eopf-playground/values.yaml`. For e.g. with sed :
...
```Bash
sed 's#processing#playground-ns#g' -i ~/rs-workflow-env/apps/prefect3-worker-eopf-playground/kustomization.yaml
```

Change the namespace in the file `~/rs-workflow-env/apps/prefect3-worker-eopf-playground/values.yaml`. For e.g. with sed :

```Bash
sed 's#processing#playground-ns#g' -i ~/rs-workflow-env/apps/prefect3-worker-eopf-playground/values.yaml
```
